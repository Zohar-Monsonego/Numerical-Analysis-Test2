...
def bisection_method(f, a, b, tol=1e-6):
    ...
    while abs(b - a) > tol and k < steps:
        ...
        if f(c) == 0:  # if f(c) equal to zero it means it is the root
            return c  # Procedure completed successfully

        if f(c) * f(a) < 0:  # if sign changed between steps it means the root is between a to c
            b = c  # changing b to c
        else:
            a = c  # it means that the root is between c to b
